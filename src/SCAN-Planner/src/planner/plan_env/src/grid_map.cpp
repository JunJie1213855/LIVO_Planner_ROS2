...
void GridMap::initMap(rclcpp::Node::SharedPtr nh)
{
  ...
  auto get_param = [&](const std::string &name, auto &val, auto def) {
    ...
    node_->get_parameter(name, val);
  };

  double x_size, y_size, z_size;
  get_param("grid_map/resolution", mp_.resolution_, -1.0);
  get_param("grid_map/sliding_map_size_x", x_size, -1.0);
  get_param("grid_map/sliding_map_size_y", y_size, -1.0);
  get_param("grid_map/sliding_map_size_z", z_size, -1.0);
  get_param("grid_map/local_update_range_x", mp_.local_update_range_(0), x_size/2.0);
  get_param("grid_map/local_update_range_y", mp_.local_update_range_(1), y_size/2.0);
  get_param("grid_map/local_update_range_z", mp_.local_update_range_(2), z_size/2.0);

  get_param("grid_map/obstacles_inflation_z_up", mp_.obstacles_inflation_z_up, -1.0);
  get_param("grid_map/obstacles_inflation_z_down", mp_.obstacles_inflation_z_down, -1.0);
  get_param("grid_map/double_cylinder_radius", mp_.double_cylinder_radius_, -1.0);
  get_param("grid_map/double_cylinder_offset", mp_.double_cylinder_offset_, 0.0);
  get_param("grid_map/map_sliding_en", mp_.map_sliding_en_, true);
  get_param("grid_map/map_sliding_thresh", mp_.map_sliding_thresh_, mp_.resolution_);

  get_param("grid_map/fx", mp_.fx_, -1.0);
  get_param("grid_map/fy", mp_.fy_, -1.0);
  get_param("grid_map/cx", mp_.cx_, -1.0);
  get_param("grid_map/cy", mp_.cy_, -1.0);

  get_param("grid_map/depth_filter_maxdist", mp_.depth_filter_maxdist_, -1.0);
  get_param("grid_map/depth_filter_mindist", mp_.depth_filter_mindist_, -1.0);
  get_param("grid_map/depth_filter_margin", mp_.depth_filter_margin_, -1);
  get_param("grid_map/k_depth_scaling_factor", mp_.k_depth_scaling_factor_, -1.0);
  get_param("grid_map/skip_pixel", mp_.skip_pixel_, -1);

  get_param("grid_map/p_hit", mp_.p_hit_, -1.0);
  get_param("grid_map/p_miss", mp_.p_miss_, -1.0);
  get_param("grid_map/p_min", mp_.p_min_, -1.0);
  get_param("grid_map/p_max", mp_.p_max_, -1.0);
  get_param("grid_map/p_occ", mp_.p_occ_, -1.0);
  get_param("grid_map/max_ray_length", mp_.max_ray_length_, -0.1);

  get_param("grid_map/vis_height", mp_.vis_height_, 0.3);
  get_param("grid_map/show_occ_time", mp_.show_occ_time_, false);

  get_param("grid_map/frame_id", mp_.frame_id_, string("world"));
  get_param("grid_map/sliding_map_frame_id", mp_.sliding_map_frame_id_, string("sliding_map"));
  get_param("grid_map/ground_height", mp_.ground_height_, 0.0);

  get_param("grid_map/sensor_type", mp_.sensor_type_, string("lidar"));
  get_param("grid_map/cloud_is_world", mp_.cloud_is_world_, true);
  get_param("grid_map/need_extrinsic", mp_.need_extrinsic_, true);
  ...
  {
    RCLCPP_ERROR_STREAM(node_->get_logger(), "[GridMap] invalid grid_map/sensor_type: " << mp_.sensor_type_
                                                                << ", fallback to lidar.");
  ...
  {
    depth_sub_.reset(new message_filters::Subscriber<sensor_msgs::msg::Image>(node_, "/grid_map/depth", rmw_qos_profile_default));
    depth_pose_sub_.reset(
        new message_filters::Subscriber<nav_msgs::msg::Odometry>(node_, "/grid_map/sensor_pose", rmw_qos_profile_default));

    sync_image_pose_.reset(new message_filters::Synchronizer<SyncPolicyImagePose>(
        SyncPolicyImagePose(100), *depth_sub_, *depth_pose_sub_));
    sync_image_pose_->registerCallback(std::bind(&GridMap::depthPoseCallback, this, _1, _2));
  }
  else if (mp_.sensor_type_ == "lidar")
  {
    lidar_pose_sub_ =
        node_->create_subscription<nav_msgs::msg::Odometry>("/grid_map/sensor_pose", 50, std::bind(&GridMap::sensorPoseCallback, this, _1));
    cloud_sub_ =
        node_->create_subscription<sensor_msgs::msg::PointCloud2>("/grid_map/cloud", 10, std::bind(&GridMap::cloudCallback, this, _1));
  }

  sliding_map_frame_sub_ =
      node_->create_subscription<nav_msgs::msg::Odometry>("/grid_map/body_pose", 50, std::bind(&GridMap::slidingMapFrameCallback, this, _1));
  ...
  depth_cloud_pub_ = node_->create_publisher<sensor_msgs::msg::PointCloud2>("/grid_map/depth_cloud", 10);
...
void GridMap::cloudCallback(const sensor_msgs::msg::PointCloud2::ConstSharedPtr img)
{
  ...
  {
    RCLCPP_WARN_THROTTLE(node_->get_logger(), *node_->get_clock(), 1000, "[GridMap] no /grid_map/sensor_pose yet for lidar cloud update.");
    return;
  }
