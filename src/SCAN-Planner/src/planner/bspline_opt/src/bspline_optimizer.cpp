...
namespace scan_planner
{
  ...
  void BsplineOptimizer::setParam(rclcpp::Node::SharedPtr nh)
  {
    auto get_param = [&](const std::string &name, auto &val, auto def) {
      if (!nh->has_parameter(name))
        nh->declare_parameter(name, def);
      nh->get_parameter(name, val);
    };

    get_param("optimization/lambda_smooth", lambda1_, -1.0);
    get_param("optimization/lambda_collision", lambda2_, -1.0);
    get_param("optimization/lambda_feasibility", lambda3_, -1.0);
    get_param("optimization/lambda_fitness", lambda4_, -1.0);

    get_param("optimization/dist0", dist0_, -1.0);
    get_param("optimization/max_vel", max_vel_, -1.0);
    get_param("optimization/max_acc", max_acc_, -1.0);

    get_param("optimization/order", order_, 3);
  }
  ...
  std::vector<std::vector<Eigen::Vector3d>> BsplineOptimizer::initControlPoints(Eigen::MatrixXd &init_points, bool flag_first_init /*= true*/)
  {
      ...
      for (double a = 1.0; a >= 0.0; a -= step_size)
      {
        ...
        if (flag_got_start && flag_got_end)
        {
          ...
          if (in_id >= 0 && out_id >= in_id && out_id < init_points.cols())
            ...
            RCLCPP_WARN(rclcpp::get_logger("bspline_optimizer"),"Skip invalid collision segment: in_id=%d, out_id=%d.", in_id, out_id);
        }
      }
    }
    ...
    for (size_t i = 0; i < segment_ids.size(); ++i)
    {
      ...
      if (ret == ASTAR_RET::SUCCESS)
      {
        ...
        if (path.size() < 2)
        {
          RCLCPP_WARN(rclcpp::get_logger("bspline_optimizer"),"a star path has less than 2 points, force return!");
          return a_star_paths;
        }
      ...
      {
        RCLCPP_ERROR(rclcpp::get_logger("bspline_optimizer"),"a star error, force return!");
        return a_star_paths;
      }
    }
    ...
    for (size_t i = 0; i < segment_ids.size(); i++)
    {
      ...
      {
        RCLCPP_WARN(rclcpp::get_logger("bspline_optimizer"),"Skip invalid A-star path while assigning control point directions.");
        continue;
      }
      ...
      else
      {
        ...
        // RCLCPP_ERROR(rclcpp::get_logger("bspline_optimizer"),"Failed to generate direction! segment_id=%d", i);
      }
    }
  ...
  bool BsplineOptimizer::check_collision_and_rebound(void)
  {
    ...
    for (int i = order_ - 1; i <= i_end; ++i)
    {
      ...
      if (occ)
      {
        ...
        {
          RCLCPP_ERROR(rclcpp::get_logger("bspline_optimizer"),"ERROR! the drone is in obstacle. This should not happen.");
          in_id = 0;
        }
        ...
        {
          RCLCPP_WARN(rclcpp::get_logger("bspline_optimizer"),"WARN! terminal point of the current trajectory is in obstacle, skip this planning.");
    ...
    if (flag_new_obs_valid)
    {
      ...
      for (size_t i = 0; i < segment_ids.size(); ++i)
      {
        ...
        if (ret == ASTAR_RET::SUCCESS)
        {
          ...
          if (path.size() < 2)
          {
            RCLCPP_WARN(rclcpp::get_logger("bspline_optimizer"),"A-star path has less than 2 points, drop this collision segment.");
        ...
        else if (ret == ASTAR_RET::SEARCH_ERR && i + 1 < segment_ids.size())
        {
          ...
          --i;
          RCLCPP_WARN(rclcpp::get_logger("bspline_optimizer"),"A-star failed on one collision segment, merge it with the next segment.");
        }
        else
        {
          RCLCPP_ERROR(rclcpp::get_logger("bspline_optimizer"),"a star error");
          segment_ids.erase(segment_ids.begin() + i);
          i--;
        }
      }
      ...
      for (size_t i = 0; i < segment_ids.size(); ++i)
      {
        if (i >= a_star_paths.size() || a_star_paths[i].size() < 2)
        {
          RCLCPP_WARN(rclcpp::get_logger("bspline_optimizer"),"Skip invalid A-star path while assigning rebound directions.");
          continue;
        }
        ...
        if (got_intersection_id >= 0)
        {
          ...
          RCLCPP_WARN(rclcpp::get_logger("bspline_optimizer"),"Failed to generate direction. It doesn't matter.");
      }
  ...
  bool BsplineOptimizer::rebound_optimize()
  {
    ...
    do
    {
      ...
      {
        //RCLCPP_WARN(rclcpp::get_logger("bspline_optimizer"),"Solver error in planning!, return = %s", lbfgs::lbfgs_strerror(result));
        flag_force_return = false;
        ...
        for (double t = tm; t < tmp * 2 / 3; t += t_step) // Only check the closest 2/3 partition of the whole trajectory.
        {
          ...
          if (flag_occ)
          {
            ...
            if (t <= bspline_interval_) // First 3 control points in obstacles!
            {
              ...
              RCLCPP_WARN(rclcpp::get_logger("bspline_optimizer"),"First 3 control points in obstacles! return false, t=%f", t);
              return false;
            }
      ...
      {
        RCLCPP_WARN(rclcpp::get_logger("bspline_optimizer"),"Solver error. Return = %d, %s. Skip this planning.", result, lbfgs::lbfgs_strerror(result));
        // while (ros::ok());
      }
  ...
  bool BsplineOptimizer::refine_optimize()
  {
    ...
    do
    {
      ...
      {
        RCLCPP_ERROR(rclcpp::get_logger("bspline_optimizer"),"Solver error in refining!, return = %d, %s", result, lbfgs::lbfgs_strerror(result));
      }
