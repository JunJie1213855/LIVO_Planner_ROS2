...
rclcpp::Subscription<scan_planner_msgs::msg::Bspline>::SharedPtr bspline_sub;
...
bool loadRequiredParam(const rclcpp::Node::SharedPtr &node, const std::string &name, double &value)
{
...
bool loadParams(const rclcpp::Node::SharedPtr &node)
{
  bool ok = true;
  if (!node->has_parameter("body_pose_topic"))
    node->declare_parameter("body_pose_topic", std::string("/quad_0/body_pose"));
  node->get_parameter("body_pose_topic", body_pose_topic);
  ok &= loadRequiredParam(node, "time_forward", time_forward);
  ok &= loadRequiredParam(node, "heading_error_threshold", heading_error_threshold);
  ok &= loadRequiredParam(node, "kp_pos", kp_pos);
  ok &= loadRequiredParam(node, "kp_yaw", kp_yaw);
  ok &= loadRequiredParam(node, "max_vx", max_vx);
  ok &= loadRequiredParam(node, "max_vy", max_vy);
  ok &= loadRequiredParam(node, "max_vyaw", max_vyaw);
  ok &= loadRequiredParam(node, "finish_dist", finish_dist);
  if (ok && max_vyaw > kMaxVYawLimit)
  {
    RCLCPP_WARN(node->get_logger(), "[closed_loop_controller] cap max_vyaw %.3f to %.3f rad/s.", max_vyaw, kMaxVYawLimit);
...
void bsplineCallback(const scan_planner_msgs::msg::Bspline::ConstSharedPtr msg)
{
  ...
  traj_id = msg->traj_id;
  exec_time = 0.0;
  last_update_time = g_node->now();
  receive_traj = true;

  RCLCPP_WARN(g_node->get_logger(), "[closed_loop_controller] received bspline traj_id=%d duration=%.3f", traj_id, traj_duration);
}
...
int main(int argc, char **argv)
{
  ...
  g_node = std::make_shared<rclcpp::Node>("closed_loop_controller");

  if (!loadParams(g_node))
    return 1;

  bspline_sub = g_node->create_subscription<scan_planner_msgs::msg::Bspline>("planning/bspline", 10, bsplineCallback);
  odom_sub = g_node->create_subscription<nav_msgs::msg::Odometry>(body_pose_topic, 20, odomCallback);
  cmd_vel_pub = g_node->create_publisher<geometry_msgs::msg::Twist>("cmd_vel", 20);
  execution_frozen_pub = g_node->create_publisher<std_msgs::msg::Bool>("planning/go2_execution_frozen", 10);
  cmd_timer = g_node->create_wall_timer(std::chrono::milliseconds(10), cmdCallback);

  last_update_time = g_node->now();
  RCLCPP_WARN(g_node->get_logger(), "[closed_loop_controller] ready.");
