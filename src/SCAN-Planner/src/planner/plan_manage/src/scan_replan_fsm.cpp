...
namespace scan_planner
{
  ...
  void SCANReplanFSM::init(rclcpp::Node::SharedPtr nh)
  {
    node_ = nh;
    ...
    /*  fsm param  */
    get_param("fsm/navi_mode", navi_mode_, -1);
    get_param("fsm/thresh_replan", replan_thresh_, -1.0);
    get_param("fsm/thresh_no_replan", no_replan_thresh_, -1.0);
    get_param("fsm/planning_horizon", planning_horizon_, -1.0);
    get_param("fsm/emergency_time_", emergency_time_, 1.0);
    get_param("fsm/fail_safe", enable_fail_safe_, true);
    get_param("fsm/max_replan_fail_count", max_replan_fail_count_, 1000);
    get_param("grid_map/obstacles_inflation_z_up", self_inflation_z_up_, 0.0);
    get_param("grid_map/obstacles_inflation_z_down", self_inflation_z_down_, 0.0);
    get_param("grid_map/double_cylinder_radius", self_double_cylinder_radius_, 0.0);
    get_param("grid_map/double_cylinder_offset", self_double_cylinder_offset_, 0.0);
    get_param("grid_map/double_cylinder_offset", body_height_, 0.4);
    get_param("grid_map/frame_id", self_inflation_frame_id_, std::string("world"));
    ...
    if (navi_mode_ == NAVI_MODE::PRESET_TARGET)
    {
      ...
      const std::string load_keypoints_cmd =
          "rosparam load " + keypoints_yaml + " " + shellQuote(node_->get_namespace());
      if (std::system(load_keypoints_cmd.c_str()) != 0)
      {
        RCLCPP_ERROR(node_->get_logger(),"[SCANReplanFSM] Failed to load keypoints_yaml: tools/keypoint.yaml");
        rclcpp::shutdown();
        return;
      }

      get_param("fsm/waypoint_num", waypoint_num_, -1);

      if (waypoint_num_ <= 0)
      {
        RCLCPP_ERROR(node_->get_logger(),"[SCANReplanFSM] navi_mode=2 requires keypoints_yaml with fsm/waypoint_num and fsm/waypoint{i}_{x,y,z}.");
        rclcpp::shutdown();
        return;
      }
      ...
      {
        get_param("fsm/waypoint" + to_string(i) + "_x", preset_waypoints_[i](0), -1.0);
        get_param("fsm/waypoint" + to_string(i) + "_y", preset_waypoints_[i](1), -1.0);
        get_param("fsm/waypoint" + to_string(i) + "_z", preset_waypoints_[i](2), -1.0);
      }
    }
    ...
    odom_sub_ = node_->create_subscription<nav_msgs::msg::Odometry>(body_pose_topic, 1, std::bind(&SCANReplanFSM::odometryCallback, this, std::placeholders::_1));
    go2_execution_frozen_sub_ = node_->create_subscription<std_msgs::msg::Bool>("/planning/go2_execution_frozen", 10, std::bind(&SCANReplanFSM::go2ExecutionFrozenCallback, this, std::placeholders::_1));
    ...
    if (navi_mode_ == NAVI_MODE::MANUAL_TARGET)
      goal_sub_ = node_->create_subscription<geometry_msgs::msg::PoseStamped>("/move_base_simple/goal", 1, std::bind(&SCANReplanFSM::rvizGoalCallback, this, std::placeholders::_1));
    else if (navi_mode_ == NAVI_MODE::PRESET_TARGET)
    {
      ...
      path_sub_ = node_->create_subscription<nav_msgs::msg::Path>("/initial_path", 1, std::bind(&SCANReplanFSM::pathCallback, this, std::placeholders::_1));
    else
  ...
  void SCANReplanFSM::pathCallback(const nav_msgs::msg::Path::ConstSharedPtr msg)
  {
    if (!msg || msg->poses.empty())
    {
      RCLCPP_WARN_THROTTLE(node_->get_logger(), *node_->get_clock(), 1000,"[pathCallback] Received empty /initial_path, ignore.");
      return;
    }
  ...
  void SCANReplanFSM::getLocalTarget()
  {
    ...
    for (t = planner_manager_->global_data_.last_progress_time_; t < planner_manager_->global_data_.global_duration_; t += t_step)
    {
      ...
      {
        RCLCPP_ERROR_STREAM(node_->get_logger(),"[getLocalTarget] last_progress_time mismatch: "
