...
namespace scan_planner
{
  ...
  void SCANPlannerManager::initPlanModules(rclcpp::Node::SharedPtr nh, PlanningVisualization::Ptr vis)
  {
    node_ = nh;
    ...
    get_param("manager/max_vel", pp_.max_vel_, -1.0);
    get_param("manager/max_acc", pp_.max_acc_, -1.0);
    get_param("manager/max_jerk", pp_.max_jerk_, -1.0);
    get_param("optimization/vel_tolerance", pp_.vel_tolerance_, 1.0);
    get_param("optimization/acc_tolerance", pp_.acc_tolerance_, 1.0);
    get_param("manager/feasibility_tolerance", pp_.feasibility_tolerance_, 0.0);
    get_param("manager/control_points_distance", pp_.ctrl_pt_dist, -1.0);
    get_param("manager/planning_horizon", pp_.planning_horizon_, 5.0);
      ...
      else // Initial path generated from previous trajectory.
      {
        ...
        if (poly_time > ts)
        {
          ...
          for (t = ts; t < poly_time; t += ts)
          {
            ...
            {
          RCLCPP_ERROR(node_->get_logger(),"pseudo_arc_length is empty, return!");
              continuous_failures_count_++;
              return false;
            }
          }
  ...
  bool SCANPlannerManager::checkDynamicFeasibility(UniformBspline position_traj)
  {
    ...
    for (double t = 0.0; t < duration + 1e-6; t += sample_dt)
    {
      ...
      {
        RCLCPP_WARN_STREAM(node_->get_logger(),"Dynamic feasibility check failed: velocity limit exceeded at t="
      ...
      {
        RCLCPP_WARN_STREAM(node_->get_logger(),"Dynamic feasibility check failed: acceleration limit exceeded at t="
