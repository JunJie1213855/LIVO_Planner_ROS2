...
void declareGetParam(rclcpp::Node *node, const std::string &name, T &val, D def) {
  if (!node->has_parameter(name))
    node->declare_parameter(name, static_cast<T>(def));
...
Maps::randomMapGenerate()
{
  ...
  declareGetParam(info.nh_private,"width_min", _w_l, 0.6);
  declareGetParam(info.nh_private,"width_max", _w_h, 1.5);
  declareGetParam(info.nh_private,"height_min", _h_l, _h_l);
  declareGetParam(info.nh_private,"height_max", _h_h, _h_h);
  declareGetParam(info.nh_private,"obstacle_number", _ObsNum, 10);
  double surface_resolution;
  declareGetParam(info.nh_private,"surface_resolution", surface_resolution, _resolution * 0.5);
...
Maps::pcl2ros()
{
  ...
  RCLCPP_INFO(info.nh_private->get_logger(),"finish: infill %lf%%",
...
Maps::perlin3D()
{
  ...
  declareGetParam(info.nh_private,"complexity", complexity, 0.142857);
  declareGetParam(info.nh_private,"fill", fill, 0.38);
  declareGetParam(info.nh_private,"fractal", fractal, 1);
  declareGetParam(info.nh_private,"attenuation", attenuation, 0.5);
  ...
  RCLCPP_INFO(info.nh_private->get_logger(),"threshold: %lf", tmp);
  ...
  RCLCPP_INFO(info.nh_private->get_logger(),"the number of points before optimization is %d", info.cloud->width);
  info.cloud->points.resize(info.cloud->width * info.cloud->height);
  pcl2ros();
}
...
{
  RCLCPP_INFO(info.nh_private->get_logger(),
  ...
  { // the remaining area is larger than or equal to 5*5, need to add both x
    ...
    int  xm    = 0;
    int  ym    = 0;
    RCLCPP_INFO(info.nh_private->get_logger(),"entered 5*5 mode");
    while (valid == false)
    {
    ...
    RCLCPP_INFO(info.nh_private->get_logger(),"finished generating maze with width %d , height %d",
             xh - xl + 1,
             yh - yl + 1);
    std::cout << maze << std::endl;
    return;
  ...
  { // the case of 3*4+
    RCLCPP_INFO(info.nh_private->get_logger(),"entered 3*4+ mode");
    int doorcount = 0;
    int ym        = 0;
    for (int i = yl; i <= yh; i++)
    {
  ...
  { // the case of 4+*3
    RCLCPP_INFO(info.nh_private->get_logger(),"entered 4+*3 mode");
    int doorcount = 0;
    int xm        = 0;
    for (int i = xl; i <= xh; i++)
    {
...
Maps::recursizeDivisionMaze(Eigen::MatrixXi& maze)
{
  ...
  RCLCPP_INFO(info.nh_private->get_logger(),"debug %d %d %d %d", sx, sy, px, py);
  ...
  if ((sy - py - 3) > 0)
    ...
    y2 = py + 1;
  RCLCPP_INFO(info.nh_private->get_logger(),"%d %d %d %d", x1, x2, y1, y2);
...
Maps::maze2D()
{
  ...
  int    addWallX;
  int    addWallY;
  declareGetParam(info.nh_private,"road_width", width, 1.0);
  declareGetParam(info.nh_private,"add_wall_x", addWallX, 0);
  declareGetParam(info.nh_private,"add_wall_y", addWallY, 0);
  declareGetParam(info.nh_private,"maze_type", type, 1);
...
Maps::Maze3DGen()
{
  ...
  declareGetParam(info.nh_private,"numNodes", numNodes, 10);
  declareGetParam(info.nh_private,"connectivity", connectivity, 0.5);
  declareGetParam(info.nh_private,"nodeRad", nodeRad, 3);
  declareGetParam(info.nh_private,"roadRad", roadRad, 2);
  RCLCPP_INFO(info.nh_private->get_logger(),"received parameters : numNodes: %d connectivity: "
  ...
  for (int i = 0; i < numNodes; i++)
  {
    ...
    RCLCPP_INFO(info.nh_private->get_logger(),"point: x: %f , y: %f , z: %f", rx, ry, rz);
